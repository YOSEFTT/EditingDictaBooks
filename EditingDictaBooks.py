...
class CreateHeadersOtZria(QWidget):
    ...
    def ot(self, text, end):
        remove = ["<b>", "</b>", "<big>", "</big>", ":", ",", ";", "[", "]", "(", ")", ".", "‚", '"', "'", "״", "”", "’", "׳", "‘", "„", "`", "´", "“", "❝", "❞", "ˮ", "″", "ʺ", "ˈ", "´", "ʹ", "′", "ʾ", "ʽ"]
    ...
    def strip_html_tags(self, text):
        html_tags = ["<b>", "</b>", "<big>", "</big>", ":", ",", ";", "[", "]", "(", ")", ".", "‚", '"', "'", "״", "”", "’", "׳", "‘", "„", "`", "´", "“", "❝", "❞", "ˮ", "″", "ʺ", "ˈ", "´", "ʹ", "′", "ʾ", "ʽ"]
...
class CreateSingleLetterHeaders(QWidget):
    ...
    def ot(self, text, end):
        remove = ["<b>", "</b>", "<big>", "</big>", "<i>", "</i>", "</small>", "</small>", "<span>", "</span>", "<br>", "</br>", "<p>", "</p>", ":", ",", ";", "[", "]", "(", ")", "{", "}", ".", '"', "'", "״", "”", "’", "׳", "‘", "„", "`", "´", "“", "❝", "❞", "ˮ", "″", "ʺ", "ˈ", "´", "ʹ", "′", "ʾ", "ʽ"]
        aa = ["ק", "ר", "ש", "ת", "תק", "תר", "תש", "תת", "תתק", "יה", "יו", "קיה", "קיו", "ריה", "ריו", "שיה", "שיו", "תיה", "תיו", "תקיה", "תקיו", "תריה", "תריו", "תשיה", "תשיו", "תתיה", "תתיו", "תתקיה", "תתקיו"]
        bb = ["ם", "ן", "ץ", "ף", "ך"]
...
class AddPageNumberToHeading(QWidget):
    ...
    def set_file_path(self, file_path: str):
        """
        מאפשר לקבוע את נתיב הקובץ מתוך מחלקה חיצונית
        """
        ...
        while i < len(content):
            ...
            if match:
                ...
                if next_line_index < len(content):
                    ...
                    pattern = r'(<[a-z]+>)?(ע["\']+?[א-ב]|עמוד [א-ב])[.,:()\[\]\'"״׳”’‘„`´“❝❞ˮ″ʺˈʹ′ʾʽ]?(</[a-z]+>)?\s?'
    ...
    def cleanText(self, file_path):
        ...
        try:
            ...
            if self.checkBoxes["remove_spaces_before"].isChecked():
                ...
                # נסיון לצורה משודרגת
                # text = re.sub(r'(?m)^(.*?)\s*\n\s*(<(?:b|i|big|small)(?:\s[^>]*)?>[^<]*)*([.,:\)\]]{1,3})(<\/(?:b|i|big|small)>[^<]*)*(?:\s+(.*)|\s*$|[\u05d0-\u05ea](.*))', lambda m: f"{m.group(1).rstrip()}{m.group(3)}{chr(10) + (m.group(2) or '') + (m.group(5).lstrip() if m.group(5) and m.group(5).strip() else m.group(6) if m.group(6) else '')}" if (len(m.group(3)) == 1 or (len(m.group(3)) > 1 and any(c in ')]' for c in m.group(3)) and (any(c in '.,:' for c in m.group(3)) or sum(1 for c in m.group(3) if c in ')]') >= 2))) and not re.search(r'</h[1-6]>\s*$', m.group(1)) and not (m.group(2) and ' ' in m.group(2)) and (m.group(5) and m.group(5).strip() or m.group(6)) and not m.group(4) else (f"{m.group(1).rstrip()}{m.group(2) or ''}{m.group(3)}{m.group(4) or ''}{chr(10) + (m.group(5).lstrip() if m.group(5) and m.group(5).strip() else m.group(6) if m.group(6) else '')}" if (len(m.group(3)) == 1 or (len(m.group(3)) > 1 and any(c in ')]' for c in m.group(3)) and (any(c in '.,:' for c in m.group(3)) or sum(1 for c in m.group(3) if c in ')]') >= 2))) and not re.search(r'</h[1-6]>\s*$', m.group(1)) and not (m.group(2) and ' ' in m.group(2)) else (m.group(1) + (chr(10) + (m.group(5).lstrip() if m.group(5) and m.group(5).strip() else m.group(6) if m.group(6) else '')) if re.search(r'</h[1-6]>\s*$', m.group(1)) else m.group(0))), text)
...
class AboutDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        date_label = QLabel("תאריך: כט טבת תשפו")
